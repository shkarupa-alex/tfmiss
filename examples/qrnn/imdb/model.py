...
from tfmiss.keras.layers import QRNN
from tfmiss.keras.layers import MapFlat
...
class ImdbModel(models.Model):
    ...
    def __init__(
        ...
        outputs = ToDense(pad_value=0.0, mask=True)(outputs)

        for i in range(n_layers):
            not_last = i != n_layers - 1
            if self.CORE_LSTM == core:
                sequence = layers.LSTM(
                    units=units, dropout=dropout, return_sequences=not_last
                )
            else:
