...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dropout", type=float, default=0.0, help="Dropout applied to layers"
    )
    parser.add_argument(
        "--layer_dropout",
        type=float,
    ...
    parser.add_argument(
        "--epochs", type=int, default=15, help="Upper epoch limit"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--qrnn_layers", type=int, default=4, help="Number of QRNN layers"
    )
    parser.add_argument(
        "--qrnn_units",
        type=int,
        default=256,
        help="Number of hidden units for QRNN layer",
    )
    parser.add_argument(
        "--lstm_layers", type=int, default=4, help="Number of LSTM layers"
    )
    parser.add_argument(
        "--lstm_units",
        type=int,
    ...
    for (core, m), color in zip(histories.items(), colors):
        pyplot.plot(
            ...
            color=color,
            dashes=[6, 2],
            label="{} ({} min)".format(core, m["time"]),
        )
        pyplot.plot(
            interval,
            m["val_loss"],
            color=color,
            label="{} val ({} min)".format(core, m["time"]),
        )
