...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_batch", type=int, default=2**11, help="Maximum batch size"
    )
    parser.add_argument(
        "--max_hidden", type=int, default=2**11, help="Maximum hidden size"
    )
    parser.add_argument(
        "--max_classes",
        type=int,
        default=2**16,
        help="Maximum number of classes + maximum number of clusters "
        "(including head one)",
    )
    parser.add_argument(
        "--repeats", type=int, default=1000, help="Number repeats to average"
    )
    parser.add_argument(
        "--device", type=str, default="/GPU:0", help="Device name"
    )
    parser.add_argument(
        "--dtype", type=str, default="float32", help="Data type"
    )
