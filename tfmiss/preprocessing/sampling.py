...
def down_sample(
    ...
    """Randomly down-sample high frequency tokens in `source` with
    `replacement` value.

    Args:
        source: string `Tensor` or `RaggedTensor` or `SparseTensor` of any
          shape, items to be sampled.
    ...
    with tf.name_scope(name or "down_sample"):
        if isinstance(source, sparse_tensor.SparseTensorValue) or isinstance(
            source, sparse_tensor.SparseTensor
        ):
            source = sparse_tensor.convert_to_tensor_or_sparse_tensor(
                source, dtype=tf.string, name=name
            )
        else:
        ...
        if not tf.string.is_compatible_with(source.dtype):
            raise RuntimeError(
                '"Source" must have dtype compatible with "string". '
...
def sample_mask(
    source, freq_vocab, threshold=1e-3, min_freq=0, seed=None, name=None
):
    ...
    with tf.name_scope(name or "sample_mask"):
        ...
        if not isinstance(freq_vocab, Counter):
            raise ValueError(
                "Frequency vocabulary should be a Counter instance"
            )
...
def sample_probs(freq_vocab, threshold=1e-3):
    ...
    "Keep probabilities" below estimated with word2vec equation from source
    code (not from paper)
    ...
    keep_probs = (
        (np.sqrt(freq_vals / total_thold) + 1.0) * total_thold / freq_vals
    )
