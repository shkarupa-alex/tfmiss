...
def cbow_context(source, window, empty, name=None):
    ...
    Args:
        ...
        window: `int`, size of context before and after target token, must
          be > 0.
    ...
    with tf.name_scope(name or "cbow_context"):
        ...
        if not ragged_tensor.is_ragged(source):
            source = ragged_tensor.RaggedTensor.from_tensor(
                source, ragged_rank=1
            )

        if source.ragged_rank != 1:
            raise ValueError("Ragged rank of `source` must equals 1")

        context_values, context_splits, context_positions = (
            tfmiss_ops.miss_cbow_context(
        ...
        context = tf.RaggedTensor.from_row_splits(
            context_values, context_splits
        )
        position = tf.RaggedTensor.from_row_splits(
            context_positions, context_splits
        )
...
def cont_bow(source, window, seed=None, name=None):
    """Generates `Continuous bag-of-words` target and context pairs from
    batched list of tokens.
    ...
    with tf.name_scope(name or "cont_bow"):
        ...
        target, context_values, context_splits, context_positions = (
            tfmiss_ops.miss_cont_bow(
                source_values=source.values,
                source_splits=source.row_splits,
                window=window,
                seed=seed1,
...
def skip_gram(source, window, seed=None, name=None):
    """Generates `Skip-Gram` target and context pairs from batched list of
    tokens.
...
def spaces_after(source, name=None):
    ...
    Args:
        source: `2-D` string `Tensor` or `RaggedTensor`, batched lists of
          "tokens with spaces" [sentences, tokens].
        name: `string`, a name for the operation (optional).
    ...
    with tf.name_scope(name or "spaces_after"):
        ...
        token_values, space_values, common_splits = (
            tfmiss_ops.miss_spaces_after(
                source_values=source.values, source_splits=source.row_splits
            )
        )
