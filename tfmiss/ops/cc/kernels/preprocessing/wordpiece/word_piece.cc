// Taken from https://raw.githubusercontent.com/tensorflow/text/v2.5.0/tensorflow_text/core/kernels/wordpiece_kernel.cc
//
...
class WordpieceTokenizeOp : public OpKernel {
 public:
  explicit WordpieceTokenizeOp(OpKernelConstruction* ctx)
      : OpKernel(ctx),
...
REGISTER_KERNEL_BUILDER(Name("Miss>WordpieceTokenize").Device(DEVICE_CPU),
                        WordpieceTokenizeOp);
ALLOW_STATEFUL_OP_FOR_DATASET_FUNCTIONS("Miss>WordpieceTokenize");
