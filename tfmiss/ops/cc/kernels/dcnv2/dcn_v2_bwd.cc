...
struct ModulatedDeformableColumnBackwardFunctor<CPUDevice, T> {
    ...
    const int deformable_group, T *grad_input, T *grad_offset, T *grad_mask) const
  {
    ...
    // TODO: thread_pool->ParallelFor assumes each cost unit is 1ns
//    thread_pool->ParallelFor(num_kernels, height_out * width_out * 1000,
//        [&](int64 start_index, int64 end_index) {
//            for (int index = start_index; index < end_index; index++)
//            {
//                modulated_deformable_col2im_coord_body<T>(
//                  index, input, offset, mask, grad,
...
//        });
            ...
            {
                modulated_deformable_col2im_body<T>(
                  index, input, offset, mask, grad,
                  batch_size, height_in, width_in, channel_in, height_out, width_out,
                  kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w, deformable_group,
...
template struct ModulatedDeformableColumnBackwardFunctor<CPUDevice, Eigen::half>;
template struct ModulatedDeformableColumnBackwardFunctor<CPUDevice, float>;
template struct ModulatedDeformableColumnBackwardFunctor<CPUDevice, double>;
...
class ModulatedDeformableColumnBackwardOp : public OpKernel {
 private:
   ...
   ModulatedDeformableColumnBackwardFunctor<Device, T> col2im_functor;
 ...
 public:
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    OP_REQUIRES(ctx, batch_size == grad_tensor->shape().dim_size(0), errors::InvalidArgument("Grad batch size is different from the input one"));
    OP_REQUIRES(ctx, height_out * width_out == grad_tensor->shape().dim_size(1), errors::InvalidArgument("Grad batch height*width is different from the expected one"));
    OP_REQUIRES(ctx, channel_out == grad_tensor->shape().dim_size(2), errors::InvalidArgument("Grad channel size is different from the expected one"));
...
#define REGISTER(TYPE)                                              \
  REGISTER_KERNEL_BUILDER(Name("Miss>ModulatedDeformableColumnBackward") \
                              .Device(DEVICE_CPU)                   \
