...
template <typename T>
struct ModulatedDeformableColumnForwardFunctor<CPUDevice, T> {
  void operator()(OpKernelContext *ctx, const T *input, const T *offset, const T *mask, const int batch_size, const int height_in, const int width_in, const int channel_in, const int height_out, const int width_out, const int kernel_h, const int kernel_w, const int pad_h, const int pad_w, const int stride_h, const int stride_w, const int dilation_h, const int dilation_w, const int deformable_group, T *column) const
  {
...
//            {
//                modulated_deformable_im2col_body<T>(
//                  index, input, offset, mask,
//                  batch_size, height_in, width_in, channel_in, height_out, width_out,
//                  kernel_h, kernel_w, pad_h, pad_w, stride_h,
//                  stride_w, dilation_h, dilation_w, deformable_group, column);
//            }
//        });
...
template struct ModulatedDeformableColumnForwardFunctor<CPUDevice, Eigen::half>;
template struct ModulatedDeformableColumnForwardFunctor<CPUDevice, float>;
template struct ModulatedDeformableColumnForwardFunctor<CPUDevice, double>;
...
class ModulatedDeformableColumnOp : public OpKernel {
 private:
   ...
   ModulatedDeformableColumnForwardFunctor<Device, T> im2col_functor;
 ...
 public:
  explicit ModulatedDeformableColumnOp(OpKernelConstruction *ctx) : OpKernel(ctx) {
    ...
    OP_REQUIRES(ctx, pad_hb >= 0 && pad_ha >= 0 && pad_wb >= 0 && pad_wa >= 0, errors::InvalidArgument("Paddings should be larger or equal to 0"));

    OP_REQUIRES_OK(ctx, ctx->GetAttr("dilation_h", &dilation_h));
    OP_REQUIRES_OK(ctx, ctx->GetAttr("dilation_w", &dilation_w));
    OP_REQUIRES(ctx, dilation_w > 0 && dilation_h > 0, errors::InvalidArgument("Dilated rates should be larger than 0"));

    OP_REQUIRES_OK(ctx, ctx->GetAttr("deformable_group", &deformable_group));
    OP_REQUIRES(ctx, deformable_group > 0, errors::InvalidArgument("Number of deformable groups should be larger than 0"));
  }
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    OP_REQUIRES(ctx, deformable_group <= input_tensor->shape().dim_size(1), errors::InvalidArgument("Number of deformable groups should be less or equals to input channel dimension size"));
    ...
    OP_REQUIRES(ctx, batch_size == offset_tensor->shape().dim_size(0), errors::InvalidArgument("Offset batch size is different from the input one"));
    OP_REQUIRES(ctx, height_out == offset_tensor->shape().dim_size(1), errors::InvalidArgument("Offset height is different from the output one"));
    OP_REQUIRES(ctx, width_out == offset_tensor->shape().dim_size(2), errors::InvalidArgument("Offset width is different from the output one"));
    OP_REQUIRES(ctx, deformable_group * kernel_h * kernel_w * 2 == offset_tensor->shape().dim_size(3), errors::InvalidArgument("Wrong offset channel size"));
    ...
    OP_REQUIRES(ctx, batch_size == mask_tensor->shape().dim_size(0), errors::InvalidArgument("Mask batch size is different from the input one"));
    OP_REQUIRES(ctx, height_out == mask_tensor->shape().dim_size(1), errors::InvalidArgument("Mask height is different from the output one"));
    OP_REQUIRES(ctx, width_out == mask_tensor->shape().dim_size(2), errors::InvalidArgument("Mask width is different from the output one"));
    OP_REQUIRES(ctx, deformable_group * kernel_h * kernel_w == mask_tensor->shape().dim_size(3), errors::InvalidArgument("Wrong mask channel size"));
    ...
    im2col_functor(ctx, input, offset, mask, batch_size, height_in, width_in, channel_in, height_out, width_out,
      kernel_h, kernel_w, pad_hb, pad_wb, stride_h, stride_w, dilation_h,
      dilation_w, deformable_group, output);
  }
};


#define REGISTER(TYPE)                                              \
  REGISTER_KERNEL_BUILDER(Name("Miss>ModulatedDeformableColumn") \
                              .Device(DEVICE_CPU)                   \
                              .TypeConstraint<TYPE>("FT"),       \
