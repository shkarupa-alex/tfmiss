    ...
    def test_value(self, dev, dt, tol):
        ...
        with tf.device(dev):
            ...
            self.assertLessEqual(
                np.abs(expected_).max(),
                np.abs(input_).max() * np.abs(mask_).max(),
            )
            ...
            with tf.GradientTape() as g:
                variables = [
                    tf.constant(v, dt) for v in [input_, offset_, mask_]
                ]
                g.watch(variables)
                result = modulated_deformable_column(
                    variables[0],
                ...
                result_grad = g.gradient(
                    result, variables, output_gradients=tf.constant(grad_, dt)
                )
