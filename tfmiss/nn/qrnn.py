...
def fo_pool(inputs, forget, initial_state=None, name=None):
    ...
    Args:
        ...
        forget: Tensor, input values in [Batch, Time, Channels] format. Usually
          in the range 0-1.
        initial_state: Tensor, initial hidden state values in [Batch, Channels]
          format.
    ...
    with tf.name_scope(name or "fo_pool"):
        ...
        if initial_state is None:
            ...
            initial_state = tf.zeros(
                (inputs_shape[0], inputs_shape[2]), dtype=inputs.dtype
            )
        else:
        ...
        return tfmiss_ops.miss_fo_pool(
            input=inputs, forget=forget, init=initial_state
        )[:, 1:]
