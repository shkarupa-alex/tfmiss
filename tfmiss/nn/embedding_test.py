...
class AdaptiveEmbeddingLookupTest(tf.test.TestCase):
    def test_max_norm_constant(self):
        ...
        transforms = [lambda embed: _transform_embedding(1, embed)] * len(
            params
        )
        embeddings = adaptive_embedding_lookup(
            params, ids, transforms, max_norm=1.0
        )
        self.assertAllEqual(embeddings, [[1.0]])

    def test_max_norm_nontrivial(self):
        params = [
            tf.constant([[2.0, 4.0], [3.0, 1.0]]),
            tf.constant([[2.0, 4.0], [3.0, 1.0]]),
        ]
        ids = tf.constant([0, 1], dtype=tf.int32)
        transforms = [lambda embed: _transform_embedding(2, embed)] * len(
            params
        )
        embeddings = adaptive_embedding_lookup(
            params, ids, transforms, max_norm=2.0
        )

        norms = tf.math.sqrt(
            tf.math.reduce_sum(embeddings * embeddings, axis=1)
        )
        normalized = embeddings / tf.stack([norms, norms], axis=1)
        self.assertAllClose(embeddings, 2 * normalized)
    ...
    def test_ids_dtype(self):
        ...
        transforms = [lambda embed: _transform_embedding(4, embed)] * len(
            params
        )
        ...
        self.assertAllEqual(embeddings32, embeddings64)
    ...
    def test_variables(self):
        ...
        adaptive_embedding_lookup(p, ids, transforms)
    ...
    def test_adaptive_no_error(self):
        ...
        expected = [
            [[1.0, 1.0, 1.0, 1.0], [3.1, 3.1, 0.0, 0.0]],
            [[1.3, 1.3, 1.3, 1.3], [4.0, 0.0, 0.0, 0.0]],
        ]
        self.assertAllClose(embeddings, expected)
    ...
    def test_adaptive_shape(self):
        ...
        self.assertAllEqual(actual_shape, [2, 1, 4])
    ...
    def test_adaptive_error(self):
        with self.assertRaisesRegexp(ValueError, "2 variable"):
            adaptive_embedding_lookup(
                [[0.0]], [0], [lambda embed: _transform_embedding(2, embed)]
            )
        with self.assertRaisesRegexp(ValueError, "corresponding transform"):
            adaptive_embedding_lookup([[[0.0]], [[0.0]]], [0], [])
        ...
        with self.assertRaisesRegexp(
            tf.errors.InvalidArgumentError, "id should be less"
        ):
            adaptive_embedding_lookup(
    ...
    def test_adaptive_ragged(self):
        ...
        embeddings = adaptive_embedding_lookup(
            params, ids, transforms
        ).to_tensor()
        expected = np.array(
            [
                [[1.0, 1.0, 1.0, 1.0], [3.1, 3.1, 0.0, 0.0]],
                [[1.3, 1.3, 1.3, 1.3], [0.0, 0.0, 0.0, 0.0]],
            ]
        )
        self.assertAllClose(embeddings, expected)
