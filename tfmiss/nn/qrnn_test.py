...
class FoPoolTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_value(self, dev, dt, tol):
        ...
        with tf.device(dev):
            ...
            initial_state = np.random.random(size=shape[:1] + shape[2:]).astype(
                dt
            )
            ...
            with tf.GradientTape() as g:
                variables = [
                    tf.constant(v, dt) for v in [inputs, forget, initial_state]
                ]
                g.watch(variables)
                result = fo_pool(variables[0], variables[1], variables[2])

                result_grad = g.gradient(
                    result, variables, output_gradients=tf.constant(grad, dt)
                )
