...
def embedding_lookup_adaptive(params, ids, transforms, max_norm=None, name=None):
    ...
    with a partitioner.
    Each element `id` of `ids` is partitioned between the elements of `params` according to the div
    `partition_strategy`.
    ...
    if not isinstance(params, (list, tuple)) or len(params) < 2:
        raise ValueError('Need at least two params')
    ...
    with tf.name_scope(name or 'embedding_lookup_adaptive') as name:
