...
def adaptive_embedding_lookup(
    params, ids, transforms, max_norm=None, name=None
):
    """Looks up `ids` in a list of embedding tensors.
    This function is used to perform parallel lookups on the list of tensors
    in `params`.  It is a generalization of `tf.gather`, where `params` is
    interpreted as a partitioning of a large embedding tensor with different
    number of rows and columns. `params` may be a `PartitionedVariable`s as
    returned by using `tf.compat.v1.get_variable()` with a partitioner.
    Each element `id` of `ids` is partitioned between the elements of `params`
    according to the `div` partition strategy.
    Id space should be the same with total number of `params` rows.
    The results of the lookup are transformed with corresponding functions from
    `transforms` and concatenated into a dense tensor. The returned tensor
    shape selected by `transforms`.
    Args:
        params: A list of P tensors of different shape, representing sharded
          embedding tensors.  Alternatively, a `PartitionedVariable`, created
          by partitioning along dimension 0.
        ids: A `Tensor` with type `int32` or `int64` containing the ids to be
          looked up in `params`.
        transforms: Functions applied to each retrieved embedding before
          concatenation. Required due to possible different embedding sizes
          of `params`.
    ...
    if isinstance(ids, tf.RaggedTensor):
        return tf.ragged.map_flat_values(
            adaptive_embedding_lookup, params, ids, transforms, max_norm, name
        )

    if not isinstance(params, (list, tuple)) or len(params) < 2:
        raise ValueError("At least 2 variables required in params")

    if not isinstance(transforms, (list, tuple)) or len(transforms) != len(
        params
    ):
    ...
    with tf.name_scope(name or "adaptive_embedding_lookup") as name:
        ...
        # Compute total_ids_capacity as the sum of dim-0 of params, then
        # assign to partitions based on a variable number of ids per partition.
        ...
        if dim_0_size_value:
            ...
            total_ids_capacity = tf.constant(
                dim_0_size_value, dtype=flat_ids.dtype
            )
        else:
            dim_0_sizes = []
            for p in range(np):
        ...
        assert_max_id = tf.debugging.assert_less(
            ...
            total_ids_capacity,
            "Invalid id. Maximum id should be less "
            "than total number of params rows",
        )
