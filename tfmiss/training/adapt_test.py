...
from tfmiss.training.adapt import adaptive_split_cost
from tfmiss.training.adapt import build_zipf_vocab
from tfmiss.training.adapt import estimate_best_splits
from tfmiss.training.adapt import generate_class_clusters
from tfmiss.training.adapt import interpolate_matmul_cost
from tfmiss.training.adapt import test_device_matmul
...
class TestDeviceMatmulTest(tf.test.TestCase):
    def test_normal_cpu(self):
        ...
        self.assertListEqual(
            [1, 2, 3, 4, 5, 7, 8, 9], device_params["hidden_sizes"]
        )
        self.assertListEqual(
            [1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17], device_params["class_sizes"]
        )
    ...
    def test_normal_gpu(self):
        ...
        self.assertListEqual(
            [1, 2, 3, 4, 5, 7, 8], device_params["hidden_sizes"]
        )
        self.assertListEqual(
            [1, 2, 3, 4, 5, 7, 8, 9, 15, 16], device_params["class_sizes"]
        )
...
class GenerateClassClustersTest(tf.test.TestCase):
    def test_normal_1(self):
        ...
        self.assertListEqual([[15, 45], [23, 37]], splits)
