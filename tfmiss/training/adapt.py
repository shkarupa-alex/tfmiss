...
def test_device_matmul(
    max_batch, max_hidden, max_classes, repeats, device, dtype
):
    """Measures matrix multiplication time: [BATCH, HIDDEN] * [HIDDEN, CLASSES].
    In case of sequences this is the same as:
      [BATCH / TIME, TIME, HIDDEN] * [HIDDEN, CLASSES]
    ...
    Returns:
        A dict with tested `batch_sizes`, `hidden_sizes` and `class_sizes`
          along with measured `cost_values` (time in seconds).
    """
    if max_classes < max_hidden:
        raise ValueError(
            "Number of classes should be greater then input logits size"
        )

    if "GPU" not in device.upper():
        tf.get_logger().warning(
            "Device matmul estimation is useful for GPUs. You ask to measure "
            "non-GPU device. Hope you know what you are doing."
        )

    physical_devices = ", ".join(
        [
            d.name.replace("physical_device:", "")
            for d in tf.config.experimental.list_physical_devices()
        ]
    )
    if device.upper() not in physical_devices:
        raise SystemError(
            "Requested device {} is not available: {}".format(
    ...
    if not (dtype.is_floating or dtype.is_complex):
        raise TypeError(
            f"Unable to test matrix multiplication time with non-floating "
            f"dtype {dtype}"
        )
    ...
    def _matmul_dim_space(max_val):
        ...
        final_space = np.concatenate(
            (rc_space, rc_space - 1, rc_space + 1, [max_val])
        )
    ...
    dense_grid = np.array(
        np.meshgrid(batch_sizes, hidden_sizes, class_sizes, indexing="ij")
    ).T.reshape([-1, 3])
    with tf.device(device):
        # Check if device has enough memory
        left = tf.random.normal(
            shape=(batch_sizes[-1], hidden_sizes[-1]), dtype=dtype
        )
        right = tf.random.normal(
            shape=(hidden_sizes[-1], class_sizes[-1]), dtype=dtype
        )
        ...
        for step, (batch_size, hidden_size, class_size) in enumerate(
            dense_grid
        ):
            left = tf.random.normal(
                shape=(batch_size, hidden_size), dtype=dtype
            )
            right = tf.random.normal(
                shape=(hidden_size, class_size), dtype=dtype
            )

            # TensorFlow initializes a GPU the first time it's used, exclude
            # from timing.
            mult = tf.matmul(left, right)
            mult.numpy()
            start = time.time()
            ...
            # The mult.numpy() call below will ensure that all enqueued
            # operations have completed (and will also copy the result to host
            # memory, so we're including a little more than just the matmul
            # operation time).
            mult.numpy()
            finish = time.time()
            mult.numpy()
            ...
            tf.get_logger().info(
                "Done {} steps of {}".format(step + 1, dense_grid.shape[0])
            )
...
def interpolate_matmul_cost(device_params):
    """Interpolates matrix multiplication time according to measurements
    provided by `test_device_matmul` function.

    Args:
        device_params: A dict with 4 keys (`batch_sizes`, `hidden_sizes`,
          `class_sizes`, `cost_values`). Device performance measurements.

    Returns:
        A linear interpolation function with signature (batch_size,
          hidden_size, classes_size);
    """
    ...
    approx_cost = LinearNDInterpolator(
        point_grid, cost_values, fill_value=np.nan, rescale=True
    )
    ...
    def _with_bounds(batch_size, hidden_size, num_classes):
        ...
        if np.isnan(value):
            raise ValueError(
                "Required point ({}, {}, {}) is out of known bounds".format(
...
def build_zipf_vocab(num_classes):
    ...
    Returns:
        A `Counter` instance with classes from 0 to num_classes - 1 and
...
def generate_class_clusters(num_tails, prob_accum, head=None):
    ...
    if head is None:
        ...
        head_split = (
            np.floor((size_space + num_tails) / 8).astype(np.int32) * 8
            - num_tails
        )
    ...
    if 0 == np.size(head):
        raise ValueError(
            "Could not generate required number of clusters. "
    ...
    for split, size in zip(head, sizes):
        ...
        for bone in subspace:
            ...
            if (
                2.1 * prob_accum[middle]
                > prob_accum[left] + 1.1 * prob_accum[right]
            ):
...
def adaptive_split_cost(
    approx_cost, prob_accum, cluster_sizes, batch_size, hidden_size, factor
):
    """Estimates computation time for adaptpive softmax split.

    Args:
        approx_cost: Function to estimate matmul for batch-hidden_size-class
          matrices.
    ...
    if np.sum(cluster_sizes).item() != len(prob_accum):
        raise ValueError(
            "Wrong inputs: Sum of cluster sizes should be equal to size of "
            "accumulated probabilities."
        )
    cluster_accum = np.cumsum(cluster_sizes)

    cost = approx_cost(
        batch_size, hidden_size, cluster_sizes[0] + len(cluster_sizes) - 1
    ...
    for i, tail_size in enumerate(cluster_sizes[1:]):
        ...
        if dim == prev_dim:
            raise ValueError(
                "Some clusters have same internal size. "
        ...
        # In most cases we can't guarantee tail batch size evenly dividable
        # by 8. So, for estimation it won't.
        tail_batch = tail_batch + 1 if 0 == tail_batch % 8 else tail_batch

        cost += approx_cost(
            tail_batch, hidden_size, dim
...
def estimate_best_splits(
    device_params, freq_vocab, num_tails, hidden_size, factor
):
    ...
    batch_sizes = [
        bs for bs in device_params["batch_sizes"] if bs < 8 or bs % 8 == 0
    ]
    batch_sizes = sorted(set(batch_sizes + [max(device_params["batch_sizes"])]))
    try:
        base_costs = [
            approx_cost(batch, hidden_size, len(prob_accum))
            for batch in batch_sizes
        ]
    except ValueError:
        base_costs = None
        tf.get_logger().warning(
            "Can't estimate non-adaptive softmax computation time. "
    ...
    for bi, batch in enumerate(batch_sizes):
        ...
        for split in all_splits:
            curr_cost = adaptive_split_cost(
                approx_cost, prob_accum, split, batch, hidden_size, factor
            )
            head_size = split[0]
            if (
                head_size not in with_costs
                or with_costs[head_size][0] > curr_cost
            ):
