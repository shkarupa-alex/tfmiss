...
def init_buckets(len2freq):
    ...
    Returns:
        List of buckets. Each bucket is a tuple (`bucket boundary`,
          `subset of len2freq`).
    """
...
def merge_buckets(bucket1, bucket2):
    ...
    Returns:
        New bucket with maximum of boundaries and joined length-to-frequency
          mappings.
    """
...
def merge_allowed(merged, buckets, min_waste, max_waste, min_aggr):
    ...
    return (
        curr_waste < min_waste
        or curr_waste < max_waste
        and curr_aggr < min_aggr
    )


def group_buckets(before, middle, after, min_waste, max_waste, min_aggr):
    """Merges buckets one by one from `before` and `after` into middle until
    merging allowed.
    ...
    while len(middle[1]) > last_size:
        ...
        allow_left = merge_allowed(
            with_left, all_buckets, min_waste, max_waste, min_aggr
        )
        allow_right = merge_allowed(
            with_right, all_buckets, min_waste, max_waste, min_aggr
        )
...
def estimate_bucket_boundaries(
    len2freq, min_waste=0.01, max_waste=0.1, min_aggr=0.01
):
    """Estimates and merges buckets from the most common (middle).
    By default tries to make buckets with more than 1% of samples and no more
    than 1% of paddings or at least  no more than 10% of paddings.
    ...
    before, middle, after = group_buckets(
        before, middle, after, min_waste, max_waste, min_aggr
    )
    result = [middle]
    ...
    while len(before):
        middle = before[-1]
        before = before[:-1]
        before, middle, _ = group_buckets(
            before, middle, result + after, min_waste, max_waste, min_aggr
        )
    ...
    while len(after):
        middle = after[0]
        after = after[1:]
        _, middle, after = group_buckets(
            result, middle, after, min_waste, max_waste, min_aggr
        )
    ...
    if not set(original.keys()) == set(restored.keys()) or not set(
        original.values()
    ) == set(restored.values()):
        raise AssertionError(
            "Estimated boundaries differs from source lengths or frequencies"
        )
...
def estimate_bucket_pipeline(bucket_boundaries, num_samples, safe=True):
    """Estimates bach sizes and reduces bucket boundaries to fit required
    number of samples per batch.

    Args:
        bucket_boundaries: pre-estimated bucket boundaries (see
          `estimate_bucket_boundaries`).
        num_samples: number of samples per batch (same as `batch size` *
          `sequence length`).
        safe: Do not allow maximum number of samples to be greater than
          `num_samples`.

    Returns:
        A tuple of (`reduced bucket boundaries`, `batch sizes`,
          `maximum boundary`).
        Bucket boundaries and batch sizes must be supplied to
    ...
    for boundary in bucket_boundaries:
        ...
        batch_size = (
            np.floor(batch_size / batch_step)
            if safe
