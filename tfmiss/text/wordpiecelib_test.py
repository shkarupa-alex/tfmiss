# Taken from https://raw.githubusercontent.com/tensorflow/text/v2.5.0/
# tensorflow_text/tools/wordpiece_vocab/wordpiece_tokenizer_learner_test.py
#
# coding=utf-8
...
"""Tests for
tensorflow_text.python.tools.wordpiece_vocab.wordpiece_tokenizer_learner_lib."""
...
from tfmiss.text import wordpiecelib as learner
...
class EnsureAllTokensExistTest(parameterized.TestCase):
    @parameterized.named_parameters(
        {
            "testcase_name": "EmptyDict",
            "input_tokens": {"a", "b", "c"},
            "output_tokens": {},
            "expected_tokens": {"a": 1, "b": 1, "c": 1},
            "include_joiner": False,
        },
        {
            "testcase_name": "SomeTokensExist",
            "input_tokens": {"a", "b", "c"},
    ...
    def testEnsureAllTokensExist(
        self, input_tokens, output_tokens, expected_tokens, include_joiner
    ):
        joiner = "##"
        new_tokens = learner.ensure_all_tokens_exist(
            input_tokens, output_tokens, include_joiner, joiner
        )
...
class GetSplitIndicesTest(parameterized.TestCase):
    @parameterized.named_parameters(
        {
            "testcase_name": "ValidWordNoJoiner",
            "word": "hello",
            "expected_indices": [2, 5],
            "include_joiner": False,
        },
        {
            "testcase_name": "ValidWordWithJoiner",
            "word": "hello",
            "expected_indices": [2, 3, 5],
            "include_joiner": True,
        },
        {
            "testcase_name": "InvalidSplitIndices",
            "word": "world",
    ...
    def testGetSplitIndices(self, word, expected_indices, include_joiner):
        ...
        indices = learner.get_split_indices(
            word, curr_tokens, include_joiner, joiner
        )
...
class GetSearchThreshsTest(parameterized.TestCase):
    @parameterized.named_parameters(
        {
            "testcase_name": "ThreshsWithinMinMax",
            "upper_thresh": 200,
            "lower_thresh": 5,
            "expected_upper": 200,
            "expected_lower": 5,
        ...
        {
            "testcase_name": "ThreshsOutsideMinMax",
            "upper_thresh": 10000,
            "lower_thresh": 2,
            "expected_upper": 292,
            "expected_lower": 3,
    ...
    def testGetSearchThreshs(
        self, upper_thresh, lower_thresh, expected_upper, expected_lower
    ):
        word_counts = [("apple", 3), ("banana", 292), ("cucumber", 5)]
        upper, lower = learner.get_search_threshs(
            word_counts, upper_thresh, lower_thresh
        )
...
class GetInputWordsTest(parameterized.TestCase):
    ...
    def testGetInputWords(self, word_counts, expected_counts):
        ...
        new_counts = learner.get_input_words(
            word_counts, reserved_tokens, max_token_length
        )
...
class FilterInputWordsTest(parameterized.TestCase):
    ...
    def testFilterInputWords(self, word_counts, expected_counts):
        ...
        max_input_tokens = 4
        filtered_counts = learner.filter_input_words(
            word_counts, allowed_chars, max_input_tokens
        )
...
class GenerateFinalVocabularyTest(absltest.TestCase):
    ...
    def testGenerateFinalVocab(self):
        final_vocab = learner.generate_final_vocabulary(
            self.reserved_tokens, self.char_tokens, self.curr_tokens
        )
...
class LearnBinarySearchTest(parameterized.TestCase):
    ...
    def testBinarySearch(
        self, word_counts, lower, upper, delta, expected_vocab, params
    ):
...
class CountWordsTest(parameterized.TestCase):
    ...
    def test_count_lists(self):
        ...
        self.assertEqual(
            counts, collections.Counter({"aaa": 3, "bb": 1, "c": 2})
        )
