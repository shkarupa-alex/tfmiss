...
def word_piece(
    ...
    Args:
        ...
        lookup_table: A lookup table implementing the LookupInterface containing the vocabulary of subwords.
        joiner_prefix: `string`, the characters prepended to a wordpiece to indicate that it is a suffix to another
            subword. Default is '##'.
        max_bytes: `int`, maximum size of input token.
        max_chars: `int`, maximum size of subwords, excluding suffix indicator. If known, providing this improves the
            efficiency of decoding long words.
        unknown_token: `string` or None, the string value to substitute for an unknown token. If set to `None`,
    ...
    if not isinstance(lookup_table, LookupInterface):
        raise TypeError('Lookup table should supports LookupInterface, got {}'.format(lookup_table))
