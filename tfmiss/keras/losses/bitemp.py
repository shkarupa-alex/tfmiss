...
def compute_normalization(activations, t, num_iters=5):
    ...
    return tf.cond(
        ...
        functools.partial(
            compute_normalization_fixed_point, activations, t, num_iters
        ),
    )
...
def _internal_bi_tempered_logistic_loss(activations, labels, t1, t2):
    ...
    if t2 == 1.0:
        normalization_constants = tf.math.log(
            tf.reduce_sum(tf.exp(activations), -1, keepdims=True)
        )
        if t1 == 1.0:
        ...
        shifted_activations = tf.nn.relu(
    ...
    if t1 == 1.0:
        ...
        logt_probs = (
            tf.pow(shifted_activations, one_minus_t1 / one_minus_t2) - 1.0
        ) / one_minus_t1
        ...
        return tf.reduce_sum(
            ...
            * (
                tf.pow(labels, beta)
                - tf.pow(shifted_activations, beta / one_minus_t2)
            ),
            -1,
        )
...
def tempered_sigmoid(activations, t, num_iters=5):
    ...
    normalization_constants = tf.cond(
        ...
        tf.equal(t, 1.0),
        lambda: tf.math.log(
            tf.reduce_sum(tf.exp(internal_activations), -1, keepdims=True)
        ),
        functools.partial(
            compute_normalization, internal_activations, t, num_iters
        ),
    )
    internal_probabilities = exp_t(
        internal_activations - normalization_constants, t
    )
...
def bi_tempered_binary_logistic_loss(
    labels, activations, t1, t2, label_smoothing=0.0, num_iters=5
):
...
def bi_tempered_logistic_loss(
    ...
    with tf.name_scope("bitempered_logistic"):
        ...
        def _custom_gradient_bi_tempered_logistic_loss(activations):
            """Bi-Tempered Logistic Loss with custom gradient.
            Args:
              activations: A multi-dimensional tensor with last dim
                `num_classes`.
            Returns:
              A loss tensor, grad.
            """
...
def sparse_bi_tempered_logistic_loss(labels, activations, t1, t2, num_iters=5):
    ...
    with tf.name_scope("sparse_bitempered_logistic"):
        ...
        loss_values = _custom_gradient_sparse_bi_tempered_logistic_loss(
            activations
        )
...
def bi_tempered_binary_logistic(
    y_true, y_pred, t1, t2, label_smoothing=0.0, num_iters=5, from_logits=False
):
    if from_logits:
    ...
    if (
        not isinstance(y_pred, (ops.EagerTensor, tf.Variable))
        and y_pred.op.type == "Sigmoid"
        and not hasattr(y_pred, "_keras_history")
    ):
        ...
        tf.get_logger().warning(
            "Unable to obtain original logits for bi_tempered_binary_logistic "
            "loss. Logits will be estimated from probabilities, but this can "
        ...
        y_pred = -tf.math.log((1.0 - y_pred) / y_pred)
...
def bi_tempered_logistic(
    ...
    if (
        ...
        tf.get_logger().warning(
            ...
            "Logits will be estimated from probabilities, but this can be "
            "numerical unstable."
        )
        epsilon_ = tf.constant(backend.epsilon(), y_pred.dtype)
        y_pred = tf.clip_by_value(y_pred, epsilon_, 1.0 - epsilon_)
...
def sparse_bi_tempered_logistic(
    y_true, y_pred, t1, t2, num_iters=5, from_logits=False
):
    if from_logits:
    ...
    if (
        ...
        # Otherwise trying to revert softmax
        tf.get_logger().warning(
            "Unable to obtain original logits for sparse_bi_tempered_logistic "
            "loss. Logits will be estimated from probabilities, but this can "
            "be numerical unstable."
        )
        epsilon_ = tf.constant(backend.epsilon(), y_pred.dtype)
...
class BiTemperedBinaryLogistic(LossFunctionWrapper):
    ...
    def __init__(
        ...
        name="bi_tempered_binary_logistic",
    ):
        super(BiTemperedBinaryLogistic, self).__init__(
            bi_tempered_binary_logistic,
            name=name,
            reduction=reduction,
...
class BiTemperedLogistic(LossFunctionWrapper):
    ...
    def __init__(
        ...
        from_logits=False,
        reduction="sum_over_batch_size",
        name="bi_tempered_logistic",
    ):
        super(BiTemperedLogistic, self).__init__(
            bi_tempered_logistic,
            name=name,
            reduction=reduction,
            from_logits=from_logits,
            t1=t1,
...
class SparseBiTemperedLogistic(LossFunctionWrapper):
    ...
    def __init__(
        ...
        from_logits=False,
        reduction="sum_over_batch_size",
        name="sparse_bi_tempered_logistic",
    ):
        super(SparseBiTemperedLogistic, self).__init__(
            sparse_bi_tempered_logistic,
            name=name,
            reduction=reduction,
            from_logits=from_logits,
            t1=t1,
