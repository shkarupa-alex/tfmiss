...
class LRFinder(callbacks.Callback):
    ...
    def __init__(self, max_steps, min_lr=1e-7, max_lr=0.01, smooth=0.98):
    ...
    def on_train_begin(self, logs=None):
        ...
        tf.get_logger().warning(
            'Don\'t forget to set "epochs=1" and "steps_per_epoch={}" '
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if current_loss is None:
            ...
            tf.get_logger().error(
                'LRFinder conditioned on "loss" which is not available. '
        ...
        if np.isnan(current_loss) or np.isinf(current_loss):
            ...
            self.model.stop_training = True
            tf.get_logger().error(
                "LRFinder got Nan/Inf loss value. Training will be stopped."
            )
            return

        # Smooth the loss
        self.avg_loss = (
            self.smooth * self.avg_loss + (1 - self.smooth) * current_loss
        )
        ...
        if self.curr_step > 0 and smooth_loss > self.best_value * 4:
            ...
            self.model.stop_training = True
            tf.get_logger().error(
                "LRFinder found loss explosion. Training will be stopped."
            )
            return
        ...
        if self.curr_step > self.max_steps:
            ...
            self.model.stop_training = True
            tf.get_logger().info(
                "LRFinder reached final step. Training will be stopped."
            )
            return

        # Set next lr (annealing exponential)
        next_lr = self.min_lr * (self.max_lr / self.min_lr) ** (
    ...
    def on_train_end(self, logs=None):
        if self.curr_step < self.max_steps:
            tf.get_logger().error(
                'LRFinder finished before "max_steps" reached. '
                "Run training with more data."
            )
        else:
            tf.get_logger().info(
                'LRFinder finished. Use ".plot()" method see the graph.'
            )
    ...
    def plot(self, skip_start=10, skip_end=5):
        ...
        try:
            ...
            raise ValueError(
                "Failed to compute gradients, there might not be enough points."
            )
