...
class WarmHoldCoolAnnihilateScheduler(LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        pred_fn_pairs = [
            ...
            # Hold on max_lr
            ((warm_stop < step) & (step <= hold_stop), lambda: max_lr),
            # Cool down from max_lr to min_lr
            (
                (hold_stop < step) & (step <= cool_stop),
                lambda: self.cool_down(
                    min_lr, max_lr, step - hold_stop, cool_stop - hold_stop
                ),
            ),
        ...
        tf.summary.scalar(
            "learning_rate", data=curr_lr, step=tf.cast(step, "int64")
        )
...
class WarmHoldCosineCoolAnnihilateScheduler(WarmHoldCoolAnnihilateScheduler):
    def __init__(
        ...
        super(WarmHoldCosineCoolAnnihilateScheduler, self).__init__(
            ...
            annih_steps=annih_steps,
            annih_factor=annih_factor,
            name=name,
        )
        ...
        first_width = cool_steps / sum(
            cosine_width**i for i in range(cosine_cycles)
        )
        if first_width <= 1:
            raise ValueError(
                'Not enough "cool_steps" for provided "cosine_cycles" '
                'and "cosine_width".'
            )
        else:
            tf.get_logger().info(
                "First cosine cooldown cycle will remain "
                "~{:.1f} steps".format(first_width)
            )
    ...
    def cool_down(self, min_lr, max_lr, step, total):
        ...
        first_width = (
            (1 + eps_100)
            * total
            / sum(self.cosine_width**i for i in range(self.cosine_cycles))
        )
        complete_fraction = step / first_width

        cycle_idx = tf.math.floor(
            tf.math.log(1.0 - complete_fraction * (1.0 - self.cosine_width))
            / tf.math.log(self.cosine_width)
        )
        current_fraction = (1.0 - self.cosine_width**cycle_idx) / (
            1.0 - self.cosine_width
        )
        complete_fraction = (
            complete_fraction - current_fraction
        ) / self.cosine_width**cycle_idx

        height_fraction = self.cosine_height**cycle_idx
        cosine_decayed = (
            0.5
            * height_fraction
            * (1.0 + tf.math.cos(math.pi * complete_fraction))
        )
