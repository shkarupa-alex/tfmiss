...
# tensorflow_text/python/keras/layers/todense.py
...
class ToDense(layers.Layer):
    ...
    def call(self, inputs):
        if isinstance(inputs, tf.RaggedTensor):
            ...
            or backend.is_keras_tensor(inputs)
            and inputs.sparse
        ):
            outputs = tf.sparse.to_dense(inputs, default_value=self.pad_value)
        elif tf.is_tensor(inputs) or backend.is_keras_tensor(inputs):
            raise TypeError("Tensor is already dense")
        else:
            raise TypeError(
                "Unexpected tensor type {}".format(type(inputs).__name__)
            )
