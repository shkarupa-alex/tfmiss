...
from tfmiss.keras.layers import AdaptiveEmbedding
from tfmiss.keras.layers import Reduction
...
class WordEmbedding(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.input_spec = InputSpec(
            min_ndim=1, dtype="string" if with_prep else "int64"
        )
        self._supports_ragged_inputs = True

        if vocabulary is not None:
            if not isinstance(vocabulary, list) or not all(
                map(lambda x: isinstance(x, str), vocabulary)
            ):
                raise ValueError(
                    'Expecting "vocabulary" to be a list of strings'
                )
            if len(vocabulary) != len(set(vocabulary)):
                raise ValueError(
                    'Expecting "vocabulary" to contain unique values'
                )
        self.vocabulary = vocabulary

        if output_dim is not None and output_dim < 1:
            raise ValueError(
                'Expecting "output_dim" to be None or greater then 0'
            )
        ...
        if reserved_words is not None:
            if not isinstance(reserved_words, list) or not all(
                map(lambda x: isinstance(x, str), reserved_words)
            ):
                raise ValueError(
                    'Expecting "reserved_words" to be a list of strings'
                )
            if len(reserved_words) != len(set(reserved_words)):
                raise ValueError(
                    'Expecting "reserved_words" to contain unique values'
                )
        self.reserved_words = reserved_words

        if embed_type not in {"dense_auto", "dense_cpu", "adapt"}:
            raise ValueError(
                'Expecting "embed_type" to be one of "dense_auto", "dense_cpu" '
                'or "adapt"'
            )
        ...
        self._reserved_words = [self.UNK_MARK] + [
            r for r in self._reserved_words if self.UNK_MARK != r
        ]

        self._vocabulary = [] if vocabulary is None else vocabulary
        self._vocabulary = self._reserved_words + [
            v for v in self._vocabulary if v not in self._reserved_words
        ]
    ...
    def vocab(self, word_counts, **kwargs):
        if not word_counts:
            raise ValueError(
                "Can't estimate vocabulary with empty word counter"
            )
    ...
    def adapt(self, inputs):
        ...
        if self.normalize_unicode:
            outputs = miss_text.normalize_unicode(
                outputs, form=self.normalize_unicode, skip=self._reserved_words
            )
        if self.lower_case:
        ...
        if self.max_len is not None:
            ...
            cutouts = tf.stack(
                [
                    miss_text.sub_string(
                        values, 0, self.max_len // 2, skip=self._reserved_words
                    ),
    ...
    def lookup(self, inputs=None):
        if self._lookup is None:
            ...
            self._lookup = layers.StringLookup(
                vocabulary=self._vocabulary,
                mask_token=None,
                oov_token=self.UNK_MARK,
            )
    ...
    def build(self, input_shape):
        ...
        if "adapt" == self.embed_type:
            self.embed = AdaptiveEmbedding(
                self.adapt_cutoff,
                self._lookup.vocabulary_size(),
                self.output_dim,
                factor=self.adapt_factor,
...
class NgramEmbedding(WordEmbedding):
    ...
    def adapt(self, inputs):
        ...
        wraps = miss_text.wrap_with(
            adapts, self.BOW_MARK, self.EOW_MARK, skip=self._reserved_words
        )
        ngrams = miss_text.char_ngrams(
            wraps, self.minn, self.maxn, self.itself, skip=self._reserved_words
        )
...
class WPieceEmbedding(WordEmbedding):
    ...
    def __init__(
        ...
        **kwargs
        ...
        _reserved_words += (
            ...
            else [r for r in reserved_words if r not in _reserved_words]
        )
    ...
    def call(self, inputs, **kwargs):
        if not self.with_prep and not isinstance(inputs, tf.RaggedTensor):
            raise ValueError(
                "Expecting `inputs` to be `RaggedTensor` instance."
            )
...
class CnnEmbedding(WordEmbedding):
    ...
    def __init__(
        ...
        adapt_factor=4,
        with_prep=False,
        embeddings_initializer=initializers.RandomUniform(-1.0, 1.0),
        **kwargs
    ):
        ...
        if (
            not kernels
            or not isinstance(kernels, (list, tuple))
            or not all(map(lambda x: isinstance(x, int), kernels))
        ):
            raise ValueError('Expecting "kernels" to be a list of integers')
        if (
            not filters
            or not isinstance(filters, (list, tuple))
            or not all(map(lambda x: isinstance(x, int), filters))
        ):
    ...
    def build(self, input_shape=None):
        def _kernel_init(kernel_size):
            if "tanh" == activations.serialize(self.activation):
                stddev = np.sqrt(1.0 / (kernel_size * self.char_dim))
        ...
        self.conv = [
            layers.Conv1D(
                f, k, padding="same", kernel_initializer=_kernel_init(k)
            )
        ...
        self.proj = (
            None
            if sum(self.filters) == self.output_dim
            else layers.Dense(self.output_dim)
        )
...
class Highway(layers.Layer):
    ...
    def build(self, input_shape=None):
        ...
        self.input_spec = InputSpec(
            ndim=max(2, len(input_shape)), axes={-1: channels}
        )

        kernel_initializer = initializers.RandomNormal(
            mean=0.0, stddev=np.sqrt(1.0 / channels)
        )

        self.carry = layers.Dense(
            channels,
            kernel_initializer=kernel_initializer,
            bias_initializer=initializers.Constant(-2.0),
            activation="sigmoid",
            dtype=self.dtype_policy,
        )
        self.carry.build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        outputs = carry_gate * transform_gate + (1.0 - carry_gate) * inputs
