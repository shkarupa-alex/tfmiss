...
from keras import constraints, initializers, layers, models, regularizers, KerasTensor
...
class AdaptiveSoftmax(layers.Layer):
    ...
    def __init__(
            ...
            bias_constraint=None, loss_reduction='sum_over_batch_size', **kwargs):
    ...
    def build(self, input_shape):
        ...
        for i in range(len(self._cutoff) - 1):
            ...
            self.tails.append([
                ...
                layers.Dropout(self.dropout, name='tail_drop_{}'.format(i), dtype=self.dtype_policy),
                layers.Dense(
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if training:
            logprobs_flat, loss = self._train_probs_loss(inputs_flat, targets_flat, mask_flat)
        else:
            logprobs_flat, loss = self._eval_probs_loss(inputs_flat, targets_flat, mask_flat)
    ...
    def _train_loss(self, inputs, targets, mask):
        ...
        for i in range(len(self._cutoff) - 1):
            ...
            tail_loss = reduce_weighted_values(tail_loss, reduction=self.loss_reduction)
            tail_loss = tf.cond(
        ...
        root_loss = reduce_weighted_values(root_loss, reduction=self.loss_reduction)
    ...
    def _train_probs_loss(self, inputs, targets, mask):
        ...
        full_loss = full_loss if mask is None else full_loss[mask]  # TODO: error?
    ...
    def _eval_probs_loss(self, inputs, targets, mask):
        ...
        full_loss = reduce_weighted_values(full_loss, reduction=self.loss_reduction)
...
class SampledSofmax(layers.Layer):
    ...
    def __init__(
            ...
            loss_reduction='sum_over_batch_size', **kwargs):
