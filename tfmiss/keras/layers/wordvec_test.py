...
class WordEmbeddingTest(testing.TestCase):
    ...
    def test_merged_vocab(self):
        ...
        self.assertAllEqual(layer._vocabulary, [layer.UNK_MARK, '~TesT~'] + vocab)
    ...
    def test_layer(self):
        ...
        self.run_layer_test(
            WordEmbedding,
            init_kwargs={'vocabulary': vocab, 'output_dim': 12, 'normalize_unicode': 'NFKC', 'lower_case': False,
        ...
        self.run_layer_test(
            WordEmbedding,
            init_kwargs={'vocabulary': vocab, 'output_dim': 12, 'normalize_unicode': None, 'lower_case': False,
                         'zero_digits': False, 'max_len': None, 'reserved_words': None, 'embed_type': 'dense_auto',
                         'adapt_cutoff': None, 'adapt_factor': 4},
            input_data=inputs,
            expected_output_dtype='float32',
        ...
        self.run_layer_test(
            WordEmbedding,
            init_kwargs={'vocabulary': vocab, 'output_dim': 12, 'normalize_unicode': 'NFKC', 'lower_case': True,
                         'zero_digits': False, 'max_len': None, 'reserved_words': None, 'embed_type': 'dense_auto',
                         'adapt_cutoff': None, 'adapt_factor': 4},
            input_data=inputs,
            expected_output_dtype='float32',
        ...
        self.run_layer_test(
            WordEmbedding,
            init_kwargs={'vocabulary': ['[UNK]'] + vocab, 'output_dim': 12, 'normalize_unicode': 'NFKC',
                         'lower_case': False,
                         'zero_digits': False, 'max_len': None, 'reserved_words': None, 'embed_type': 'dense_auto',
                         'adapt_cutoff': None, 'adapt_factor': 4},
            input_data=inputs,
        ...
        self.run_layer_test(
            WordEmbedding,
            init_kwargs={'vocabulary': vocab + ['[UNK]'], 'output_dim': 12, 'normalize_unicode': 'NFKC',
                         'lower_case': False,
                         'zero_digits': False, 'max_len': None, 'reserved_words': None, 'embed_type': 'dense_auto',
                         'adapt_cutoff': None, 'adapt_factor': 4},
            input_data=inputs,
...
class BpeEmbeddingTest(testing.TestCase):
    def test_reserved_words(self):
        layer = BpeEmbedding()
        self.assertAllEqual(layer._reserved_words, [layer.UNK_MARK, layer.UNK_CHAR])

        layer = BpeEmbedding(reserved_words=['~TesT~'])
        self.assertAllEqual(layer._reserved_words, [layer.UNK_MARK, layer.UNK_CHAR, '~TesT~'])

    def test_merged_vocab(self):
        vocab = ['the', '##o', 'f', '##u', '##m', '##s', 'o', '##v', '##er', 'l', '##a', '##y', 'd', '##g']
        layer = BpeEmbedding(vocab)
        self.assertAllEqual(layer._vocabulary, [layer.UNK_MARK, layer.UNK_CHAR] + vocab)

        layer = BpeEmbedding(vocab, reserved_words=['~TesT~'])
        self.assertAllEqual(layer._vocabulary, [layer.UNK_MARK, layer.UNK_CHAR, '~TesT~'] + vocab)
    ...
    #     self.run_layer_test(
    #         BpeEmbeddingWrap,
    #         init_kwargs={'vocabulary': vocab, 'output_dim': 12, 'reduction': 'mean', 'reserved_words': None,
    #                      'embed_type': 'dense_auto', 'adapt_cutoff': None, 'adapt_factor': 4},
    #         input_data=inputs,
    #         expected_output_dtype='float32',
    #         expected_output_shape=(8, 12)
...
class CnnEmbeddingTest(testing.TestCase):
    def test_reserved_words(self):
        layer = CnnEmbedding()
        self.assertAllEqual(layer._reserved_words, [layer.UNK_MARK, layer.BOW_MARK, layer.EOW_MARK])

        layer = CnnEmbedding(reserved_words=['~TesT~'])
        self.assertAllEqual(layer._reserved_words, [layer.UNK_MARK, layer.BOW_MARK, layer.EOW_MARK, '~TesT~'])

    def test_merged_vocab(self):
        vocab = ['e', 'o', 't', 'h']
        layer = CnnEmbedding(vocab)
        self.assertAllEqual(layer._vocabulary, [layer.UNK_MARK, layer.BOW_MARK, layer.EOW_MARK] + vocab)

        layer = CnnEmbedding(vocab, reserved_words=['~TesT~'])
        self.assertAllEqual(layer._vocabulary, [layer.UNK_MARK, layer.BOW_MARK, layer.EOW_MARK, '~TesT~'] + vocab)
    ...
    #     self.run_layer_test(
    #         CnnEmbeddingWrap,
    #         init_kwargs={'vocabulary': vocab, 'output_dim': 12, 'filters': [32, 32, 64, 128, 256, 512, 1024],
